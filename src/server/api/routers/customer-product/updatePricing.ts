...
export const updatePricing = protectedProcedure
  .input(inputSchema)
  .output(outputSchema)
  .mutation(async ({ input, ctx }) => {
    ...
    try {
      const response = await fetch(
        `${env.STRAPI_API_URL}/api/customer-products/${input.id}`,
        {
          method: "PUT",
          headers: {
      ...
      console.error("Failed to update customer product pricing", error);
      ...
      if (!(error instanceof Response)) {
        throw new TRPCError({
          ...
          message:
            "An unknown error occurred while updating customer product pricing via Strapi API. Please try again later.",
        });
      }
